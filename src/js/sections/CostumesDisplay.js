...
export default class CostumesDisplay extends Component {
    ...
    async componentDidMount() {
        if (this.props.costumeId) {
            const response = await fetch("http://localhost:8080/costumes/" + this.props.costumeId);
            const data = await response.json();

            for (let i = 0; i < data.similarProducts[0].sections.length; i++) {
                let section = data.similarProducts[0].sections[0];
                this.setState({sectionTitle: section.name});
                this.setState({subtext: section.subtext});
            }
            await this.processRequest(data.similarProducts);
        } else if (this.props.idSection) {
            const response = await fetch("http://localhost:8080/costumes/section/" + this.props.idSection);
            const data = await response.json();
                    ...
                    <div className="container">
                        ...
                        <div className="row">
                            {this.state.costumes.map((costume, index) => {
                                return <div key={costume.id} className="col-md-3 text-center">
